...
function Home() {
  ...
  useEffect(() => {
  fetch('https://mymoviz-part5-backend-rho.vercel.app/movies')
    ...
    .then(data => {
      const formatedData = data.movies.map(movie => {
        ...
        return { title: movie.title, poster, voteAverage: movie.vote_average, voteCount: movie.vote_count, overview: overview };
      });
